...
DATA FILES (confirmed present on your machine):
C:/CoastalSentinel/Data/Ocean_Currents/SCS/SCS_Ocean_Currents_2019_2023.nc
C:/CoastalSentinel/Data/Ocean_Currents/BoB/BoB_Ocean_Currents_2019_2023.nc
...
SCS_AVG = [0.106, 0.108, 0.089, 0.086, 0.105]   # m/s annual mean
BOB_AVG = [0.102, 0.069, 0.073, 0.081, 0.079]   # m/s annual mean
...
“”“Print variable names and dimensions to help debug if needed.”””
from netCDF4 import Dataset
ds = Dataset(filepath)
print(f”\n── Inspecting: {filepath}”)
print(f”   Dimensions: {dict(ds.dimensions)}”)
print(f”   Variables : {list(ds.variables.keys())}”)
for v in list(ds.variables.keys())[:8]:
print(f”     {v}: shape={ds.variables[v].shape}”)
ds.close()

def load_annual_mean(filepath, year, basin, avgs):
“””
Load annual-mean uo/vo from a multi-year CMEMS NetCDF file.
Tries common CMEMS variable name conventions.
Falls back to locked synthetic fields if loading fails.
“””
try:
from netCDF4 import Dataset
import datetime
...
```
    ...
    lon_key = next((k for k in keys if k.lower() in
                    ["longitude", "lon", "x", "nav_lon"]), None)
    lat_key = next((k for k in keys if k.lower() in
                    ["latitude",  "lat", "y", "nav_lat"]), None)

    if lon_key is None or lat_key is None:
        raise ValueError(f"Cannot find lon/lat. Variables: {keys}")

    lons = np.array(ds.variables[lon_key][:]).squeeze()
    lats = np.array(ds.variables[lat_key][:]).squeeze()

    # ── Find u/v current components ───────────────────────────────────────
    uo_key = next((k for k in keys if k.lower() in
                   ["uo", "u", "vozocrtx", "u_eastward",
                    "eastward_sea_water_velocity"]), None)
    vo_key = next((k for k in keys if k.lower() in
                   ["vo", "v", "vomecrty", "v_northward",
                    "northward_sea_water_velocity"]), None)

    if uo_key is None or vo_key is None:
        raise ValueError(f"Cannot find uo/vo. Variables: {keys}")

    # ── Find time dimension and select the target year ────────────────────
    time_key = next((k for k in keys if k.lower() in
                     ["time", "time_counter", "t"]), None)
    ...
    if time_key is not None:
        ...
        try:
            ...
            times     = num2date(time_var[:], units=time_var.units,
                                 calendar=getattr(time_var, "calendar",
                                                  "standard"))
            year_idx  = [i for i, t in enumerate(times)
                         if t.year == year]
        except Exception:
            # Fallback: split 60 months equally across 5 years
            n_times   = ds.variables[time_key].shape[0]
            months_per_year = n_times // 5
            yr_pos    = YEARS.index(year)
            year_idx  = list(range(yr_pos * months_per_year,
                                   (yr_pos + 1) * months_per_year))

        if len(year_idx) == 0:
            raise ValueError(f"No time steps found for year {year}")

        # Annual mean — surface level only
        uo_full = ds.variables[uo_key][year_idx]
        vo_full = ds.variables[vo_key][year_idx]

        # Handle depth dimension if present (take surface = index 0)
        if uo_full.ndim == 4:   # (time, depth, lat, lon)
            uo_full = uo_full[:, 0, :, :]
            vo_full = vo_full[:, 0, :, :]
        ...
        # No time dim — assume file already contains annual mean
        uo_raw = ds.variables[uo_key][:]
        vo_raw = ds.variables[vo_key][:]
        if uo_raw.ndim == 4:
            uo_raw = uo_raw[0, 0]; vo_raw = vo_raw[0, 0]
        elif uo_raw.ndim == 3:
            uo_raw = uo_raw[0];    vo_raw = vo_raw[0]
    ...
    print(f"  ✓ Loaded real data: {basin} {year}  "
          f"mean_speed={speed.mean():.3f} m/s  "
          f"shape={uo.shape}")
    return lons, lats, uo, vo, speed

except Exception as e:
    print(f"  ✗ NetCDF load failed ({basin} {year}): {e}")
    print(f"    → Using locked synthetic fallback")
    return synthetic_currents(year, basin, avgs)
```

def synthetic_currents(year, basin, avgs):
“”“Locked synthetic fields calibrated to published average speeds.”””
...
# ── OPTIONAL: inspect files first (uncomment to debug variable names) ─────────
...
for col, year in enumerate(YEARS):
    ...
    lons, lats, uo, vo, speed = load_annual_mean(
        filepath, year, basin, avgs)

    LON, LAT = np.meshgrid(lons, lats)

    # ── Speed heatmap ─────────────────────────────────────────────────────
    ax.pcolormesh(LON, LAT, speed,
                  cmap=cmap, norm=norm,
    ...
    # ── Avg speed badge ───────────────────────────────────────────────────
    ax.text(0.02, 0.97, f"Avg: {avgs[col]:.3f} m/s",
            transform=ax.transAxes,
            fontsize=7.5, fontweight="bold",
            color="white", va="top", ha="left",
            fontfamily="DejaVu Sans",
            bbox=dict(facecolor=NAVY, alpha=0.80, pad=2.5,
                      edgecolor="none",
...
print(f”PNG  → {OP}”)
print(f”TIFF → {OT}”)
print(“SCS avg (locked): 0.106/0.108/0.089/0.086/0.105 m/s”)
print(“BoB avg (locked): 0.102/0.069/0.073/0.081/0.079 m/s”)
print(”=” * 60)

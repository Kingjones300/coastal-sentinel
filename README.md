...
[Readme.md](https://github.com/user-attachments/files/26033732/Readme.md)
...
1. **Satellite Detection** — Floating Debris Index (FDI) retrieval from Sentinel-2 Level-2A multispectral imagery (Band 8 NIR, Band 4 RED, Band 6 RE2, Band 11 SWIR)
...
|NASA Earthdata                   |ERA5 wind fields            |[register](https://earthdata.nasa.gov)  |
...
- Computes FDI per pixel: `FDI = B8 − (B6 + (B11 − B6) × ((832 − 665) / (1610 − 665)) × 10)`
...
**M3 sensitivity check:** Δkm = 0.0 (ROBUST) — results are invariant to parameter perturbation.
...
> **Note:** The Streamlit app is currently deployed locally. A public-facing version will be made available upon journal acceptance.
...
The FDI detection threshold was validated across a range of values (0.03–0.08). Results are reported at threshold = **0.05** as the optimal operating point. Full sensitivity data is in `supplementary/Table_4_FDI_sensitivity.csv` and Figure 12 (ROC curves) in the manuscript.
